...
WAF_PAYLOAD = "AND 1=1 UNION ALL SELECT 1,NULL,'<script>alert(\"XSS\")</script>',table_name FROM information_schema.tables WHERE 2>1--/**/; EXEC xp_cmdshell('cat ../../../etc/passwd')#"
def getwaf(url=''):
	result_str = 'URL错误'
	if url.startswith('https://') or url.startswith('http://'):
		...
		if url:
			try:
				res = requests.get(url+payload, timeout=4)
...
if __name__ == '__main__':
	print(getwaf('http://browser.360.cn/'))